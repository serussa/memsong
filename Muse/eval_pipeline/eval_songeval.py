...
def main():
    ...
    # Load MuQ from local snapshot
    _muq_path = args.muq
    if not os.path.isfile(os.path.join(_muq_path, "config.json")):
        _local = "/root/autodl-tmp/hf_cache/hub/models--OpenMuQ--MuQ-large-msd-iter/snapshots/0562a57814f6f8bbd9fdea0a25921a2fce1a841a"
        if os.path.isfile(os.path.join(_local, "config.json")):
            _muq_path = _local
    _muq_cfg = json.load(open(os.path.join(_muq_path, "config.json")))
    muq = MuQ(_muq_cfg).to(device).eval()
    _sd_file = os.path.join(_muq_path, "model.safetensors")
    if not os.path.isfile(_sd_file):
        _sd_file = os.path.join(_muq_path, "pytorch_model.bin")
    _sd = load_file(_sd_file, device="cpu") if _sd_file.endswith(".safetensors") else torch.load(_sd_file, map_location="cpu")
    muq.load_state_dict(_sd, strict=False)
