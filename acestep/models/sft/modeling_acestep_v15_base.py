...
class AceStepDiTLayer(GradientCheckpointingLayer):
    ...
    def __init__(self, config: AceStepConfig, layer_idx: int, use_cross_attention: bool = True,
        ...
        if self.use_phase_memory:
            pm_dim = getattr(config, "phase_mem_dim", None) or getattr(config, "phase_memory_dim", None)
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            if getattr(module, "_pm_safe_output", False):
                # PhaseMemory proj_out — must STAY zero for safe Step-0 injection.
            ...
            # Zero-init output projection for safe injection at Step 0.
            nn.init.zeros_(module.out.weight)
            if module.out.bias is not None:
                nn.init.zeros_(module.out.bias)
